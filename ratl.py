...
pj1 = {'lf': { 'hip':   ( -45.0,  1.0, -30.0, 120.0),
               'thigh': (  25.0,  1.0, -70.0,  65.0),
               'shin':  ( -65.0,  1.0, -45.0, 145.0),
               'foot_sw': ( 0 ) },
       'rf': { 'hip':   (  45.0,  1.0, -120.0, 30.0),
               'thigh': (  25.0, -1.0, -70.0,  65.0),
               'shin':  ( -65.0, -1.0, -45.0, 145.0),
               'foot_sw': ( 0 ) },
...
class Ratl:
    ...
    def prepare_pca(self, device="/dev/i2c-1", i2c_addr=0x40, pwm_freq=60.0):
        u""" prepare PCA9685 controller and servo motors.
        connect to i2c device, set RS304MD servo channels.
        返り値: probopy.EA1_OK, or probopy.ea1_status_enum。
        """
        self._pca1 = probopy.Pca9685()
        rc = self._pca1.init(device, i2c_addr, pwm_freq)
        if rc == probopy.EA1_OK:
            rc = self.set_pwm(self._pca1, pj_pwms1)
        return rc
